...
export function getTodaysDate(format = 'YYYY.MM.DD') {
  return dayjs().format(format);
}

export function getTodaysDateInUtc(format = FULL_DATE_24_HOUR_TIME_FORMAT) {
  return dayjs().utc().format(format);
}

export function getAllDateElementsInNumberFormat() {
  const now = dayjs().utc();
  return {
    year: now.year(),
    month: now.month() + 1,
