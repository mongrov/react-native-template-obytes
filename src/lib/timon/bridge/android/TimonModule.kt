...
class TimonModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun initTimon(storagePath: String, bucketInterval: Int, userName: String, promise: Promise) {
        try {
   ...
   @ReactMethod
    fun query(dbName: String, sqlQuery: String, userName: String?, limitPartitions: Int, promise: Promise) {
        Thread {
            try {
    ...
    @ReactMethod
    fun cloudSinkParquet(dbName: String, tableName: String, promise: Promise) {
        try {
    ...
    @ReactMethod
    fun getSyncMetadata(dbName: String, tableName: String, promise: Promise) {
        try {
            val result = nativeGetSyncMetadata(dbName, tableName)
            promise.resolve(result)
        } catch (e: Exception) {
            promise.reject("Error fetching sync metadata", e)
        }
    }

    @ReactMethod
    fun getAllSyncMetadata(dbName: String, promise: Promise) {
        try {
            val result = nativeGetAllSyncMetadata(dbName)
            promise.resolve(result)
        } catch (e: Exception) {
            promise.reject("Error fetching all sync metadata", e)
        }
    }

    @ReactMethod
    fun cloudFetchParquetBatch(usernames: ReadableArray, dbNames: ReadableArray, tableNames: ReadableArray, dateRange: ReadableMap?, promise: Promise) {
        try {
    ...
    @ReactMethod
    fun preloadTables(dbName: String, tableNames: ReadableArray, userName: String?, promise: Promise) {
        Thread {
            try {
                // Convert ReadableArray to Array<String>
                val tableNamesArray = Array(tableNames.size()) { i ->
